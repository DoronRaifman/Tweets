...
class Config:
    ...
    def get_config():
        ...
        try:
            ...
            config_file_name = os.path.join(base_path, 'SysConfig.toml')
            config_file_name = os.path.abspath(config_file_name)
            config = toml.load(config_file_name)
        except FileNotFoundError as e:
            print(f"@@@ Missing system configuration file 'SysConfig.toml'. Copy to local file from SysConfig_.toml")
            config = None
        return config
